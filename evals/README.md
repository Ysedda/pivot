...
Phase-completion attestation. Checking boxes proves I did the work. Passing evals proves I learnt something.

## Why evals exist

The single biggest failure mode of self-paced learning is fooling yourself. You read about Cilium's eBPF dataplane, you nod, you tick the box, and three weeks later you can't explain it on a whiteboard. Evals close that loop: every phase ends with verifiable, time-boxed exercises with explicit pass criteria.

## Eval flavors

- **Drill** — hands-on. Do X, get Y, in time T.
- **Whiteboard** — explain it cold, no notes. Ideally to another human; otherwise write it as a doc under `tasks/whiteboards/0X/<topic>.md`.
...
Each `evals/0X-*.md` opens with a **Passing bar** section: the explicit criterion for "phase done". Tick the phase's final deliverable in `tasks/todo.md` only after passing the bar. This is the discipline that turns the program from a checklist into an attested skill set.

## Claude as the chaos monkey

For break-fix drills, instruct Claude to administer the eval. Example prompt:

> "Administer break-fix drill #N from `evals/0X-*.md`. Pick one of the listed variants, apply it to the lab, then go silent. Don't reveal what you changed until I either fix it or formally give up. Time limit: T minutes."

Claude picks a variant within the drill's spec, applies it (with explicit permission for any destructive Bash), and waits. This makes the break-fix a genuine surprise — closer to a real on-call page than a self-imposed scenario.

For drills that demand external grading (mock interview, README skim test), recruit a human. Don't grade your own behavioral interview.

## Where artefacts live

- Drill run notes & timings: `tasks/lessons.md` § "Phase 0X evals" — minimal log per drill (drill, date, pass/partial/fail, notes).
...
Run any time you doubt readiness — these aren't tied to a phase exit, they're litmus tests for skills the program assumes you've absorbed by the end.
...
- Don't fudge it. Add to `tasks/todo.md` § Slip log: `2026-MM-DD — checkpoint <X> failed in evals/0X-*.md, blocked by <gap>`.
- Schedule a focused 2–4 hour study session on the specific gap.
- Retry. If it fails again, the phase plan probably under-covered the topic — note in `tasks/lessons.md` so future-me knows.
